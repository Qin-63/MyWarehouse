...
void USART1_IRQHandler(void){ //串口1中断服务程序（固定的函数名不能修改）	
	...
	if(USART_GetITStatus(USART1, USART_IT_RXNE) != RESET){  //接收中断(接收到的数据必须是0x0d 0x0a结尾)		
		key =USART_ReceiveData(USART1);//读取接收到的数据
		//printf("%c",key); //把收到的数据发送回电脑		  
	} 
} 
#endif	
...
void USART2_IRQHandler(void){ //串口2中断服务程序（固定的函数名不能修改）	
	...
	OSIntEnter();    
#endif
	if(USART_GetITStatus(USART2, USART_IT_RXNE) != RESET)  //USART_IT_RXNE接收中断(接收到的数据必须是0x0d 0x0a结尾)
		{
		Res =USART_ReceiveData(USART2);	//读取接收到的数据
		
		if((USART2_RX_STA&0x8000)==0)//接收未完成
			{
			if(USART2_RX_STA&0x4000)//接收到了0x0d   0000 1101
				{
				...
				if(Res==0x0d)USART2_RX_STA|=0x4000;
					...
					{
					USART2_RX_BUF[USART2_RX_STA&0X3FFF]=Res ;   //0011 1111
					USART2_RX_STA++;
...
//        }            
//    }
	if(USART_GetITStatus(USART3, USART_IT_RXNE) != RESET)  //USART_IT_RXNE接收中断(接收到的数据必须是0x0d 0x0a结尾)
		{
		Res =USART_ReceiveData(USART3);	//读取接收到的数据
		
		if((USART3_RX_STA&0x8000)==0)//接收未完成
			{
			if(USART3_RX_STA&0x4000)//接收到了0x0d   0000 1101
				{
				...
				if(Res==0x0d)USART3_RX_STA|=0x4000;
					...
					{
					USART3_RX_BUF[USART3_RX_STA&0X3FFF]=Res ;   //0011 1111
					USART3_RX_STA++;
