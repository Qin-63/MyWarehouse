...
uint8_t  TIM1CH1_CAPTURE_STA1=0;	//输入捕获状态
u32 TIM1CH1_CAPTURE_VAL1=0;

void TIM1_CC_IRQHandler(void)
{
	if((TIM1CH1_CAPTURE_STA1&0X80)==0)//还未成功捕获	
	{	  
		if (TIM_GetITStatus(TIM1, TIM_IT_Update) != RESET)
		 
		{	    
			if(TIM1CH1_CAPTURE_STA1&0X40)//已经捕获到高电平了
			{
				if((TIM1CH1_CAPTURE_STA1&0X3F)==0X3F)//高电平太长了
				{
					TIM1CH1_CAPTURE_STA1|=0X80;//标记成功捕获了一次
	...
	if (TIM_GetITStatus(TIM1, TIM_IT_CC1) != RESET)//捕获1发生捕获事件
		{	
			if(TIM1CH1_CAPTURE_STA1&0X40)		//捕获到一个下降沿 		
			{	  			
				TIM1CH1_CAPTURE_STA1|=0X80;		//标记成功捕获到一次上升沿
				TIM1CH1_CAPTURE_VAL1=TIM_GetCapture1(TIM1);
		   		TIM_OC1PolarityConfig(TIM1,TIM_ICPolarity_Rising); //CC1P=0 设置为上升沿捕获
			}else  								//还未开始,第一次捕获上升沿
			{
				TIM1CH1_CAPTURE_STA1=0;			//清空
				TIM1CH1_CAPTURE_VAL1=0;
	 			TIM_SetCounter(TIM1,0);
				TIM1CH1_CAPTURE_STA1|=0X40;		//标记捕获到了上升沿
